...
class PriorityQueue(Queue):
    """Cola de prioridad basada en lista ordenada según f(n)."""

    def __init__(self, order=min, f=lambda x: x):
        """order: min o max; f: función de prioridad"""
        self.A = []
        self.f = f
        self.order = order  # min para menor primero, max para mayor primero

    def append(self, item):
        """Inserta el elemento en la posición correcta según f."""
        priority = self.f(item)
        inserted = False
        for i in range(len(self.A)):
            if (self.order == min and priority < self.f(self.A[i])) or \
               (self.order == max and priority > self.f(self.A[i])):
    ...
    def pop(self):
        """Extrae el elemento con mayor prioridad (el primero de la lista)."""
        if self.A:
            return self.A.pop(0)
