...
class Node:
    ...
    def expand(self, problem):
        ...
        for (act, nxt) in problem.successor(self.state):
            NODOS_GENERADOS += 1
            sucesores.append(
                Node(nxt, self, act, problem.path_cost(self.path_cost, self.state, act, nxt))
            )
        return sucesores
...
def graph_search(problem, fringe):
    global NODOS_VISITADOS, NODOS_GENERADOS

    closed = {}  # ← inicializar el diccionario de estados visitados

    nodo_inicial = Node(problem.initial)
    NODOS_GENERADOS += 1   # ← contar el inicial
    fringe.append(nodo_inicial)
...
def branch_and_bound_search(problem):
    """Branch & Bound: siempre expandir el nodo con menor path_cost"""
    global NODOS_GENERADOS, NODOS_VISITADOS
    closed = {}
    start_node = Node(problem.initial)
    NODOS_GENERADOS += 1

    # prioridad = path_cost
    frontier = PriorityQueue(f=lambda n: n.path_cost)
    ...
    while len(frontier) > 0:
        ...
        if node.state not in closed or node.path_cost < closed[node.state]:
...
def branch_and_bound_with_heuristic(problem):
    """A* search: path_cost + heurística"""
    global NODOS_GENERADOS, NODOS_VISITADOS
    closed = {}
    start_node = Node(problem.initial)
    NODOS_GENERADOS += 1

    # prioridad = path_cost + heurística
    frontier = PriorityQueue(f=lambda n: n.path_cost + problem.h(n))
    frontier.append(start_node)
